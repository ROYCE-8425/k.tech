...
class CandidateJobController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('welcome', compact('jobs', 'locations', 'sector'));
    }
    ...
    public function show($id)
    {
        ...
        return view('jobs.show', compact('job', 'currentCandidate', 'alreadyApplied'));
    }
    ...
    public function apply(Request $request, $id)
    {
        ...
        // Add score to message if available
        if ($aiScore !== null) {
            $message .= sprintf(' 🤖 Điểm phù hợp: %.1f/10', $aiScore);
        }

        return redirect()->route('jobs.show', $job->id)
            ->with('status', $message)
            ->with('ai_score', $aiScore)
            ->withFragment('apply-form');
    }

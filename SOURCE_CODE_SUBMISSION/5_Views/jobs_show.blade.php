<x-layouts.app title="{{ $job->title }} - {{ $job->company->name ?? 'Công ty' }}">
    <div class="max-w-5xl mx-auto">
        ...
        {{-- DEBUG: Status={{ session('status') ? 'YES' : 'NO' }} | Score (10)={{ session('ai_score') ?? 'NULL' }} --}}
        ...
        @if(session('status'))
            <div class="mb-6 p-6 rounded-3xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white shadow-2xl shadow-emerald-500/30" 
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1">
                        <h3 class="font-bold text-lg mb-1">✅ Nộp đơn thành công!</h3>
                        <p class="text-emerald-50 mb-3">Chúng tôi sẽ liên hệ với bạn sớm nhất.</p>
                            ...
                            <div class="mt-4 p-5 rounded-2xl bg-white/20 border-2 border-white/40 backdrop-blur-sm">
                                ...
                                <div class="pt-3 border-t border-white/30">
                                    <p class="text-sm text-white/90 flex items-start gap-2">
                                        ...
                                        <span>CV của bạn được hệ thống phân tích dựa trên yêu cầu công việc. Nhà tuyển dụng sẽ xem xét kỹ hơn!</span>
                                    </p>
                                </div>
                            </div>
                        @else
                            <p class="text-sm text-white/80">CV của bạn đang được xử lý...</p>
                        @endif
                    </div>
                    ...
                    @if(Auth::check() && (Auth::user()->role === 'admin' || Auth::user()->role === 'recruiter'))
                        ...
                        <div class="bg-white rounded-3xl shadow-xl shadow-gray-200/50 overflow-hidden" id="admin-panel">
                            ...
                            <div class="bg-gradient-to-r from-emerald-600 to-teal-600 p-6">
                                <h2 class="text-xl font-bold text-white mb-1">🛠️ Quản lý tuyển dụng</h2>
                                <p class="text-emerald-100 text-sm">Xem và quản lý ứng viên cho vị trí này</p>
                            </div>

                            <div class="p-6 space-y-4">
                                <!-- View Applications -->
                                <a href="{{ route('admin.jobs.applications', $job->id) }}" 
                                   class="flex items-center justify-center w-full py-4 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold text-lg hover:shadow-xl hover:shadow-indigo-500/30 hover:scale-[1.02] transition-all duration-300">
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <!-- Compare Candidates -->
                                <a href="{{ route('admin.jobs.compare', $job->id) }}" 
                                   class="flex items-center justify-center w-full py-3 rounded-xl bg-purple-100 text-purple-700 font-semibold hover:bg-purple-600 hover:text-white transition-all duration-300">
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <!-- Export PDF -->
                                <a href="{{ route('admin.jobs.export-pdf', $job->id) }}" 
                                   class="flex items-center justify-center w-full py-3 rounded-xl bg-red-100 text-red-700 font-semibold hover:bg-red-600 hover:text-white transition-all duration-300">
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <!-- Job Stats -->
                            <div class="px-6 pb-6">
                                <div class="p-4 rounded-xl bg-gradient-to-r from-gray-50 to-indigo-50 border border-indigo-100">
                                    <p class="text-sm text-gray-500 mb-2">Thống kê nhanh</p>
                                    <div class="flex items-center justify-between">
                                        <span class="text-gray-700 font-medium">Đã đăng</span>
                                        <span class="text-indigo-600 font-bold">{{ $job->created_at->diffForHumans() }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div id="cv_form_section_{{ $job->id }}" class="hidden">
                                    ...
                                    <div id="scoringProgress_{{ $job->id }}" class="hidden p-4 rounded-2xl bg-gradient-to-r from-purple-50 to-indigo-50 border-2 border-purple-200">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="flex-1">
                                                <p class="font-bold text-purple-900">🤖 Hệ thống đang chấm điểm CV của bạn...</p>
                                                <p class="text-sm text-purple-700 mt-1">Phân tích độ phù hợp với yêu cầu công việc</p>
                                            </div>
                                        </div>
                                    </div>

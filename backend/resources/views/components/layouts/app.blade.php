...
<body class="font-sans antialiased bg-gradient-to-br from-slate-50 via-white to-indigo-50/30 min-h-screen">
    ...
    {{-- ═══════════════════ NAVIGATION ═══════════════════ --}}
    <nav class="fixed left-0 right-0 z-50 glass border-b border-white/20 shadow-sm {{ config('app.demo_mode') ? 'top-7' : 'top-0' }}">
                ...
                <div class="hidden md:flex items-center space-x-1">
                    ...
                    @auth
                        @if(Auth::user()->role === 'candidate')
                            ...
                            <a href="{{ route('candidate.profile') }}" class="px-4 py-2 rounded-lg text-gray-600 hover:text-indigo-600 hover:bg-indigo-50 font-medium text-sm transition-all">
                                Hồ sơ
                            </a>
                        @else
                    ...
                    @auth
                        ...
                        <div class="relative" x-data="{ open: false }">
                            ...
                            <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-48 py-1.5 bg-white rounded-xl shadow-xl border border-gray-100 text-sm">
                                @if(Auth::user()->role === 'candidate')
                                    <a href="{{ route('candidate.profile') }}" class="flex items-center px-4 py-2 text-gray-700 hover:bg-indigo-50 hover:text-indigo-600">Hồ sơ của tôi</a>

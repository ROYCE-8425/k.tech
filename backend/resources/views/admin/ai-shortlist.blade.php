<x-layouts.app title="AI Shortlist - {{ $job->title }}">
    ...
    <div class="mb-8">
        <a href="{{ route('admin.dashboard') }}" class="inline-flex items-center text-gray-500 hover:text-indigo-600 mb-6 group transition-colors">
            ...
            <span class="font-medium">{{ __('Quay lại Dashboard') }}</span>
        </a>
            ...
            <div>
                ...
                <div class="flex items-center space-x-4 text-gray-500">
                    <span class="inline-flex items-center">
                        ...
                        {{ count($shortlist) }} {{ __('ứng viên được xếp hạng') }}
                    </span>
                </div>
            </div>
        </div>
    ...
    <div class="glass-panel rounded-2xl p-4 mb-6">
        <div class="flex flex-wrap items-center justify-center gap-6 text-sm">
            ...
            <div class="flex items-center">
                ...
                <span class="text-gray-600">{{ __('Phù hợp cao (≥80)') }}</span>
            </div>
            <div class="flex items-center">
                <span class="w-3 h-3 rounded-full bg-amber-500 mr-2"></span>
                <span class="text-gray-600">{{ __('Phù hợp vừa (60–79)') }}</span>
            </div>
            <div class="flex items-center">
                <span class="w-3 h-3 rounded-full bg-red-500 mr-2"></span>
                <span class="text-gray-600">{{ __('Phù hợp thấp (<60)') }}</span>
            </div>
            <div class="flex items-center">
                <span class="w-3 h-3 rounded-full bg-gray-300 mr-2"></span>
                <span class="text-gray-600">{{ __('Lỗi / Chưa có') }}</span>
            </div>
            <div class="flex items-center border-l border-gray-300 pl-4 ml-2">
                ...
                <span class="text-gray-600">{{ __('Mới tính') }}</span>
            </div>
            <div class="flex items-center">
                <span class="inline-block w-3 h-3 rounded-full bg-orange-200 border-2 border-orange-400 mr-2"></span>
                <span class="text-gray-600">{{ __('Cũ (>7 ngày)') }}</span>
            </div>
            <div class="flex items-center border-l border-gray-300 pl-4 ml-2">
                <span class="text-gray-500 font-medium">💬 {{ __('Phản hồi:') }}</span>
            </div>
            ...
            <div class="flex flex-col md:flex-row md:items-center gap-3 md:justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-sm text-gray-500">
                        {{ __('Đã chọn:') }} <span id="ai-refresh-selected-count" class="font-semibold text-violet-700">0</span>
                    </span>
                </div>
                <button type="submit"
                    ...
                    {{ __('Tính lại AI hồ sơ đã chọn') }}
                </button>
            </div>
        </div>

        <form id="ai-refresh-selected-form"
              action="{{ route('admin.jobs.ai-refresh-selected', $job->id) }}"
              method="POST"
              class="hidden"
                ...
                <div class="glass-card rounded-2xl overflow-hidden animate-fade-in {{ $index === 0 ? 'ring-2 ring-violet-200' : '' }}" style="animation-delay: {{ $index * 0.03 }}s;"
                    ...
                    <div class="flex flex-col md:flex-row md:items-center p-5 gap-4 cursor-pointer hover:bg-gray-50/50 transition-colors select-none"
                        ...
                        <div class="flex items-center gap-4 min-w-0">
                            ...
                            <div class="flex-shrink-0 w-10 h-10 rounded-xl {{ $scoreBg }} flex items-center justify-center font-bold text-lg shadow-md">
                                {{ $index + 1 }}
                            </div>
                            ...
                            <div class="min-w-0">
                                <h3 class="font-extrabold text-gradient-primary truncate text-lg">{{ $item['candidate_name'] }}</h3>
                        ...
                        <div class="flex items-center gap-3 md:ml-auto">
                            ...
                            <div class="w-16 h-16 rounded-2xl {{ $scoreBg }} flex flex-col items-center justify-center shadow-md">
                                @if($score !== null)
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="flex items-center justify-between pt-2 border-t border-gray-100">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <form action="{{ route('admin.applications.ai-refresh', $item['application_id']) }}" method="POST" class="inline-block"

...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.app','data' => ['title' => ''.e($job->title).' - '.e($job->company->name ?? 'Công ty').'']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => ''.e($job->title).' - '.e($job->company->name ?? 'Công ty').'']); ?>
    <div class="max-w-5xl mx-auto">
        <!-- Back Button -->
        <a href="<?php echo e(route('home')); ?>" class="inline-flex items-center text-gray-500 hover:text-indigo-600 mb-8 group transition-colors">
        ...
        <?php if(config('app.demo_mode')): ?>
            <?php if(auth()->guard()->check()): ?>
                <?php if(Auth::user()->role === 'candidate'): ?>
                    <?php if(!empty($alreadyApplied) && !empty($persistedAdvisory)): ?>
                    ...
                    <?php elseif(!empty($alreadyApplied)): ?>
                ...
                <?php elseif(Auth::user()->role === 'recruiter' || Auth::user()->role === 'admin'): ?>
                    ...
                    <div class="mb-6 flex items-center gap-3 p-3 rounded-2xl bg-purple-50 border border-purple-200 animate-fade-in">
                        ...
                        <div class="text-sm">
                            ...
                            <?php if($jobAppCount > 0 && $jobAiCount > 0): ?>
                                <span class="text-purple-600"><?php echo e($jobAppCount); ?> ứng viên · <?php echo e($jobAiCount); ?> có kết quả AI sẵn — nhấn "AI Shortlist" bên phải.</span>
                            <?php elseif($jobAppCount > 0): ?>
                                <span class="text-purple-600"><?php echo e($jobAppCount); ?> ứng viên chưa có AI — nhấn "AI Shortlist" để chấm điểm.</span>
                            <?php else: ?>
        ...
        <?php if(session('status')): ?>
            <?php $aiAdvisory = session('ai_advisory'); ?>
            <div class="mb-6 p-6 rounded-3xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white shadow-2xl shadow-emerald-500/30" style="background: linear-gradient(to right, #10b981, #14b8a6);" 
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-emerald-50 mb-3">🤖 AI đã phân tích CV của bạn cho vị trí: <strong><?php echo e($job->title); ?></strong></p>
                        
                        <?php if($aiAdvisory && isset($aiAdvisory['fit_score'])): ?>
                            <?php
                            ...
                            <div class="mt-4 p-5 rounded-2xl bg-indigo-900/40 border border-indigo-200/30 backdrop-blur-md">
                                <div class="flex items-center justify-between mb-3">
                                    <div>
                                        ...
                                        <div class="flex items-baseline gap-2">
                                            <span class="text-5xl font-black text-white"><?php echo e(number_format($score, 1)); ?></span>
                                            <span class="text-2xl font-bold text-indigo-200">/10</span>
                                        </div>
                                    </div>
                                    <div class="text-center">
                                        <div class="text-5xl mb-2"><?php echo e($emoji); ?></div>
                                        <p class="text-lg font-bold bg-white/20 text-white px-3 py-1 rounded-full"><?php echo e($label); ?></p>
                                        <?php if(!empty($aiAdvisory['rank_label'])): ?>
                                            <p class="text-xs text-indigo-100 mt-1"><?php echo e($aiAdvisory['rank_label']); ?></p>
                                        <?php endif; ?>
                                    </div>
                                </div>

                                
                                <?php if(!empty($aiAdvisory['matched_skills'])): ?>
                                    <div class="pt-3 border-t border-white/20">
                                        <p class="text-xs font-bold text-white/90 mb-2">✅ Kỹ năng phù hợp:</p>
                                        <div class="flex flex-wrap gap-1.5">
                                            <?php $__currentLoopData = array_slice($aiAdvisory['matched_skills'], 0, 8); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $skill): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <span class="px-2 py-0.5 rounded-lg bg-emerald-400/30 text-white text-xs font-medium"><?php echo e($skill); ?></span>
                                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                            <?php if(count($aiAdvisory['matched_skills']) > 8): ?>
                                                <span class="px-2 py-0.5 rounded-lg bg-white/10 text-white/80 text-xs">+<?php echo e(count($aiAdvisory['matched_skills']) - 8); ?></span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                <?php endif; ?>
                                ...
                                <?php if(!empty($aiAdvisory['missing_skills'])): ?>
                                    <div class="pt-3 mt-3 border-t border-white/20">
                                        <p class="text-xs font-bold text-amber-200 mb-2">💡 Có thể bổ sung vào CV:</p>
                                        <div class="flex flex-wrap gap-1.5">
                                            <?php $__currentLoopData = array_slice($aiAdvisory['missing_skills'], 0, 6); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $skill): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <span class="px-2 py-0.5 rounded-lg bg-amber-400/25 text-amber-100 text-xs font-medium"><?php echo e($skill); ?></span>
                                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                        </div>
                                    </div>
                                <?php endif; ?>
                                ...
                                <?php if(!empty($aiAdvisory['risk_flags'])): ?>
                                    <div class="pt-3 mt-3 border-t border-white/20">
                                        <p class="text-xs font-bold text-amber-200 mb-2">📋 Lưu ý:</p>
                                        <ul class="text-xs text-white/90 space-y-1">
                                            <?php $__currentLoopData = array_slice($aiAdvisory['risk_flags'], 0, 4); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $flag): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                ...
                                <?php if(($aiAdvisory['confidence_label'] ?? '') === 'low'): ?>
                                    <div class="pt-3 mt-3 border-t border-white/20">
                                        <p class="text-xs text-amber-200 flex items-start gap-1.5">
                                            <span>⚠️</span>
                                            <span>AI chưa đủ thông tin để đánh giá chính xác. Hãy <a href="<?php echo e(route('candidate.dashboard')); ?>" class="underline font-semibold">bổ sung hồ sơ</a> để tăng độ chính xác.</span>
                                        </p>
                                    </div>
                                <?php endif; ?>
                                ...
                                <div class="pt-3 mt-3 border-t border-white/30">
                                    <p class="text-sm text-white/90 flex items-start gap-2">
                                        ...
                                        <span>Kết quả này được AI phân tích tự động. Nếu bạn muốn điểm cao hơn, hãy <a href="<?php echo e(route('candidate.dashboard')); ?>" class="underline font-semibold">cập nhật hồ sơ</a> và thử nộp lại.</span>
                                    </p>
                                </div>
                            </div>
                        <?php elseif(session('ai_score')): ?>
                            ...
                            <div class="mt-4 p-5 rounded-2xl bg-white/20 border-2 border-white/40 backdrop-blur-sm">
                                <div class="flex items-center justify-between mb-3">
                                    <div>
                                        ...
                                        <div class="flex items-baseline gap-2">
                                            <span class="text-5xl font-black"><?php echo e(number_format(session('ai_score'), 1)); ?></span>
        ...
        <?php if(session('error')): ?>
            <div class="mb-6 p-5 rounded-2xl bg-red-50 border-2 border-red-200 text-red-800" 
                ...
                <div class="flex items-start gap-3">
                    ...
                    <p class="flex-1 font-semibold"><?php echo e(session('error')); ?></p>
                ...
                <div class="relative bg-white rounded-3xl shadow-xl shadow-gray-200/50 overflow-hidden">
                        ...
                        <div class="absolute -bottom-10 left-8">
                            <div class="w-20 h-20 rounded-2xl bg-white shadow-xl flex items-center justify-center overflow-hidden">
                                <?php if($job->company && $job->company->logo_path): ?>
                                    <img src="<?php echo e(asset('storage/' . $job->company->logo_path)); ?>" alt="<?php echo e($job->company->name ?? 'Company'); ?>" class="w-full h-full object-cover">
                                <?php else: ?>
                    ...
                    <div class="p-8 pt-14">
                        <div class="flex flex-wrap items-start justify-between gap-4 mb-4">
                            <div>
                                <p class="text-indigo-600 font-semibold mb-2"><?php echo e($job->company->name ?? 'Công ty'); ?></p>
                                <h1 class="text-3xl font-bold text-gray-900"><?php echo e($job->title); ?></h1>
                            </div>
                            <?php if($job->created_at->diffInDays() < 7): ?>
                        ...
                        <div class="flex flex-wrap gap-3 mt-6">
                            <?php if($job->location): ?>
                                <div class="inline-flex items-center px-4 py-2 rounded-xl bg-gray-100 text-gray-700">
                                    ...
                                    <span class="font-medium"><?php echo e($job->location); ?></span>
                                </div>
                            <?php endif; ?>
                            <?php if($job->salary_min || $job->salary_max): ?>
                                <div class="inline-flex items-center px-4 py-2 rounded-xl bg-emerald-100 text-emerald-700">
                                    ...
                                    <span class="font-semibold">
                                        <?php if($job->salary_min && $job->salary_max): ?>
                                            <?php echo e(number_format($job->salary_min)); ?> - <?php echo e(number_format($job->salary_max)); ?> VNĐ
                                        <?php elseif($job->salary_min): ?>
                                            Từ <?php echo e(number_format($job->salary_min)); ?> VNĐ
                                        <?php else: ?>
                                            Tới <?php echo e(number_format($job->salary_max)); ?> VNĐ
                                        <?php endif; ?>
                                    </span>
                                </div>
                            <?php endif; ?>
                            <div class="inline-flex items-center px-4 py-2 rounded-xl bg-indigo-100 text-indigo-700">
                                ...
                                <span class="font-medium"><?php echo e($job->created_at->format('d/m/Y')); ?></span>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="p-6 space-y-5">
                            <div class="flex items-center gap-4">
                                <div class="w-16 h-16 rounded-2xl bg-white shadow flex items-center justify-center overflow-hidden">
                                    <?php if($companyLogoPath): ?>
                                        <img src="<?php echo e(asset('storage/' . $companyLogoPath)); ?>" alt="<?php echo e($companyName); ?>" class="w-full h-full object-cover">
                                    <?php else: ?>
                                        <span class="text-2xl font-bold gradient-text"><?php echo e(strtoupper(substr($companyName, 0, 1))); ?></span>
                                    <?php endif; ?>
                                </div>
                                <div class="min-w-0">
                                    <p class="text-lg font-bold text-gray-900 truncate"><?php echo e($companyName); ?></p>
                                    <?php if($companyAddress): ?>
                                        <p class="text-sm text-gray-500 truncate"><?php echo e($companyAddress); ?></p>
                                    <?php endif; ?>
                                </div>
                            </div>

                            <?php if($companyWebsite): ?>
                                <a href="<?php echo e($companyWebsite); ?>" target="_blank" rel="noopener" class="inline-flex items-center justify-center w-full py-3 rounded-xl bg-indigo-50 text-indigo-700 font-semibold hover:bg-indigo-600 hover:text-white transition-all">
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <?php if($companyDescription): ?>
                                <div class="rounded-2xl border border-gray-100 bg-gray-50/40 p-4">
                                    ...
                                    <p class="text-sm text-gray-600 leading-relaxed"><?php echo nl2br(e($companyDescription)); ?></p>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>

                    <?php if(Auth::check() && (Auth::user()->role === 'admin' || Auth::user()->role === 'recruiter')): ?>
                            ...
                            <div class="px-6 pb-6">
                                <div class="p-4 rounded-xl bg-gradient-to-r from-gray-50 to-violet-50 border border-violet-100">
                                    ...
                                    <div class="flex items-center justify-between">
                                        ...
                                        <span class="text-violet-600 font-bold"><?php echo e($job->created_at->diffForHumans()); ?></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    <?php else: ?>
                        <?php if(auth()->guard()->guest()): ?>
                            ...
                            <div class="bg-white rounded-3xl shadow-xl shadow-gray-200/50 overflow-hidden" id="apply-form">
                                ...
                                <div class="p-6 space-y-4">
                                    <a href="<?php echo e(route('login')); ?>" class="inline-flex items-center justify-center w-full py-4 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold hover:shadow-xl hover:scale-[1.02] transition-all" style="background: linear-gradient(to right, #4f46e5, #9333ea);">
                                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <p class="text-center text-sm text-gray-500">
                                        Chưa có tài khoản? <a href="<?php echo e(route('register')); ?>" class="text-indigo-600 font-semibold hover:underline">Đăng ký miễn phí</a>
                                    </p>
                                </div>
                            </div>
                        <?php else: ?>
                            <?php if(!empty($alreadyApplied) && Auth::user()->role === 'candidate'): ?>
                                ...
                                <div class="bg-white rounded-3xl shadow-xl shadow-gray-200/50 overflow-hidden" id="apply-form">
                                    ...
                                    <div x-data="{
                                        ...
                                        cvInfo: <?php echo e(json_encode($cvExtracted ?? [])); ?>,
                                        editMode: false,
                                        questions: <?php echo e(json_encode($activeQuestions)); ?>,
                                        currentQIndex: 0,
                                        messages: [],
                                        currentInput: '',
                                        isSubmitting: false,
                                        ...
                                        submitFollowup() {
                                            ...
                                            document.getElementById('followupForm_<?php echo e($job->id); ?>').submit();
                                        },
                                        ...
                                        <div x-show="step === 2" x-transition class="flex flex-col">
                                            ...
                                            <form action="<?php echo e(route('jobs.ai-followup', $job->id)); ?>" method="POST" id="followupForm_<?php echo e($job->id); ?>" class="hidden">
                                                <?php echo csrf_field(); ?>
                                                <?php $__currentLoopData = ['phone', 'years_experience', 'primary_role', 'key_skills', 'education_level', 'english_level', 'portfolio_url', 'github_url']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $field): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <input type="hidden" name="followup_<?php echo e($field); ?>" id="followup_<?php echo e($field); ?>">
                                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                            </form>
                                        </div>
                                        ...
                                        <div x-show="step === 3" x-transition>
                                            <?php if($activeAdvisory && isset($activeAdvisory['fit_score'])): ?>
                                                <?php
                                                ...
                                                <div class="p-6">
                                                    <div class="text-center mb-4">
                                                        <div class="text-4xl mb-2"><?php echo e($emoji); ?></div>
                                                        <div class="flex items-baseline justify-center gap-1">
                                                            <span class="text-4xl font-black" style="color: <?php echo e($color); ?>;"><?php echo e(number_format($score, 1)); ?></span>
                                                            <span class="text-xl font-bold text-gray-400">/10</span>
                                                        </div>
                                                        <p class="text-sm font-bold mt-1" style="color: <?php echo e($color); ?>;"><?php echo e($label); ?></p>
                                                    </div>

                                                    <?php if(!empty($activeAdvisory['matched_skills'])): ?>
                                                        <div class="p-3 rounded-xl border border-gray-100 mb-3">
                                                            ...
                                                            <div class="flex flex-wrap gap-1">
                                                                <?php $__currentLoopData = $activeAdvisory['matched_skills']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $skill): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                                    <span class="px-2 py-0.5 rounded-lg text-xs font-medium" style="background: #d1fae5; color: #065f46;"><?php echo e($skill); ?></span>
                                                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                                            </div>
                                                        </div>
                                                    <?php endif; ?>

                                                    <?php if(!empty($activeAdvisory['missing_skills'])): ?>
                                                        <div class="p-3 rounded-xl border border-gray-100 mb-3">
                                                            <p class="text-xs font-semibold text-gray-500 mb-2">❌ Kỹ năng thiếu</p>
                                                            <div class="flex flex-wrap gap-1">
                                                                <?php $__currentLoopData = $activeAdvisory['missing_skills']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $skill): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                                    <span class="px-2 py-0.5 rounded-lg text-xs font-medium" style="background: #fee2e2; color: #991b1b;"><?php echo e($skill); ?></span>
                                                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                                            </div>
                                                        </div>
                                                    <?php endif; ?>
                                            ...
                                            <div class="px-6 pb-6 space-y-3">
                                                <a href="<?php echo e(route('candidate.applications')); ?>" class="inline-flex items-center justify-center w-full py-3 rounded-xl text-white font-semibold transition-all hover:scale-[1.02]" style="background: linear-gradient(135deg, #6366f1, #8b5cf6);">
                                                    Xem đơn ứng tuyển của tôi
                                                </a>
                                                <a href="<?php echo e(route('home')); ?>" class="inline-flex items-center justify-center w-full py-3 rounded-xl bg-gray-100 text-gray-700 font-semibold hover:bg-gray-200 transition-all">
                                                    Tìm việc khác
                                                </a>
                                            </div>
                                        </div>
                            ...
                            <?php elseif(Auth::user()->role === 'candidate'): ?>
                                ...
                                <!-- Form Body -->
                                <form action="<?php echo e(route('jobs.apply', $job->id)); ?>" method="POST" enctype="multipart/form-data" class="p-6 space-y-5" id="applyForm_<?php echo e($job->id); ?>">
                                    <?php echo csrf_field(); ?>
                                    ...
                                    <input type="hidden" name="cv_mode" id="cv_mode_<?php echo e($job->id); ?>" value="<?php echo e(old('cv_mode', 'upload')); ?>">

                                                                <!-- Full Name -->
                                                                <div>
                                                                        <label for="full_name_<?php echo e($job->id); ?>" class="block text-sm font-semibold text-gray-700 mb-2">
                                        Họ và tên <span class="text-red-500">*</span>
                                    </label>
                                    <input type="text" 
                                           name="full_name" 
                                                                                     id="full_name_<?php echo e($job->id); ?>" 
                                                                                     value="<?php echo e(old('full_name', Auth::user()->name ?? '')); ?>"
                                           class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all input-modern bg-gray-50 <?php $__errorArgs = ['full_name'];
...
unset($__errorArgs, $__bag); ?>
                                    ...
                                    <!-- Email -->
                                    <div>
                                        <label for="email_<?php echo e($job->id); ?>" class="block text-sm font-semibold text-gray-700 mb-2">
                                            Email <span class="text-red-500">*</span>
                                        </label>
                                        <input type="email" 
                                               name="email" 
                                               id="email_<?php echo e($job->id); ?>" 
                                               value="<?php echo e(old('email', Auth::user()->email ?? '')); ?>"
                                               readonly
                                               class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all input-modern bg-gray-50 <?php $__errorArgs = ['email'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-400 <?php unset($message);
...
unset($__errorArgs, $__bag); ?>
                                ...
                                <!-- Phone -->
                                <div>
                                    <label for="phone_<?php echo e($job->id); ?>" class="block text-sm font-semibold text-gray-700 mb-2">
                                        Số điện thoại
                                    </label>
                                    <input type="tel" 
                                           name="phone" 
                                           id="phone_<?php echo e($job->id); ?>" 
                                               value="<?php echo e(old('phone', $currentCandidate->phone ?? '')); ?>"
                                           ...
                                           placeholder="0912 345 678">
                                                                                                ...
                                                                                                <!-- CV Upload -->
                                <div id="cv_upload_section_<?php echo e($job->id); ?>">
                                    ...
                                    <div class="relative"
                                         ...
                                         @drop.prevent="dragging = false; if ($event.dataTransfer.files.length) { document.getElementById('cv_file_<?php echo e($job->id); ?>').files = $event.dataTransfer.files; document.getElementById('cv_file_<?php echo e($job->id); ?>').dispatchEvent(new Event('change')); }">
                                        <input type="file" 
                                               name="cv_file" 
                                               id="cv_file_<?php echo e($job->id); ?>" 
                                               accept=".doc,.docx,.pdf"
                                               class="sr-only">
                                        <label for="cv_file_<?php echo e($job->id); ?>" 
                                               :class="dragging ? 'border-indigo-500 bg-indigo-50/50 scale-[1.02]' : 'border-gray-300'"
                                               class="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed rounded-2xl cursor-pointer hover:border-indigo-500 hover:bg-indigo-50/50 transition-all duration-300 <?php $__errorArgs = ['cv_file'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-400 <?php unset($message);
...
unset($__errorArgs, $__bag); ?>">
                                            <div class="flex flex-col items-center justify-center pt-5 pb-6" id="upload-placeholder_<?php echo e($job->id); ?>">
                                            ...
                                            <div class="hidden flex-col items-center justify-center pt-5 pb-6" id="upload-success_<?php echo e($job->id); ?>">
                                                <div class="w-12 h-12 rounded-xl bg-emerald-100 flex items-center justify-center mb-3">
                                                    <svg class="w-6 h-6 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                                    </svg>
                                                </div>
                                                <p class="text-sm text-emerald-600 font-medium" id="file-name-display_<?php echo e($job->id); ?>">File đã chọn</p>
                                            </div>
                                        </label>
                                    </div>
...
unset($__errorArgs, $__bag); ?>
                                                                    ...
                                                                    <!-- Submit Button -->
                                    <button type="submit" 
                                            id="submitBtn_<?php echo e($job->id); ?>"
                                        ...
                                        <svg class="w-5 h-5 mr-2" id="submitIcon_<?php echo e($job->id); ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path>
                                        </svg>
                                        <svg class="animate-spin w-5 h-5 mr-2 hidden" id="submitSpinner_<?php echo e($job->id); ?>" fill="none" viewBox="0 0 24 24">
                                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                        </svg>
                                        <span id="submitText_<?php echo e($job->id); ?>">Gửi đơn ứng tuyển</span>
                                    </button>

                                    <!-- Scoring Loading State -->
                                    <div id="scoringProgress_<?php echo e($job->id); ?>" class="hidden p-4 rounded-2xl bg-gradient-to-r from-purple-50 to-indigo-50 border-2 border-purple-200" style="background: linear-gradient(to right, #faf5ff, #eef2ff);">
    ...
    <!-- CV Builder Dialog -->
    <dialog id="cvDialog_<?php echo e($job->id); ?>" class="rounded-3xl p-0 w-full max-w-2xl">
        <div class="bg-white rounded-3xl overflow-hidden">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-indigo-100 text-sm">
                            <?php if($currentCandidate && !empty($currentCandidate->profile_data['cv_quick'])): ?>
                    ...
                    <button type="button" onclick="closeCvDialog_<?php echo e($job->id); ?>()" class="w-10 h-10 rounded-xl bg-white/15 text-white hover:bg-white/25 transition-all">✕</button>
                </div>
            </div>
            ...
            <div class="p-6 space-y-6">
                <div class="rounded-2xl border border-gray-100 bg-gray-50/40 p-5">
                    ...
                    <textarea id="selfDescriptionInput_<?php echo e($job->id); ?>" rows="4" class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-indigo-500 focus:outline-none transition-all" placeholder="VD: Backend dev 2 năm, mạnh Laravel/MySQL, thích sản phẩm..."><?php echo e(old('self_description', $currentCandidate?->profile_data['cv_quick']['self_description'] ?? '')); ?></textarea>
                ...
                <div class="rounded-2xl border border-gray-100 bg-blue-50/50 p-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select id="englishLevel_<?php echo e($job->id); ?>" data-cert-field="english_level" class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:outline-none">
                        ...
                        <div>
                            ...
                            <input type="number" id="toeicScore_<?php echo e($job->id); ?>" data-cert-field="toeic_score" class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:outline-none" placeholder="VD: 850" min="0" max="990">
                        ...
                        <div>
                            ...
                            <input type="number" id="ieltsScore_<?php echo e($job->id); ?>" data-cert-field="ielts_score" step="0.5" class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:outline-none" placeholder="VD: 7.0" min="0" max="9">
                        ...
                        <div>
                            ...
                            <select id="certifications_<?php echo e($job->id); ?>" data-cert-field="certifications" multiple class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:outline-none" size="3">
                    ...
                    <div class="mt-4">
                        ...
                        <div class="flex items-center gap-3">
                            <input type="number" id="yearsOfExperience_<?php echo e($job->id); ?>" data-cert-field="years_experience" class="flex-1 px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:outline-none" placeholder="VD: 3" min="0" max="50" step="0.5">
                ...
                <div class="rounded-2xl border border-gray-100 bg-gray-50/40 p-5">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <button type="button" onclick="addEducationRow_<?php echo e($job->id); ?>()" class="px-4 py-2 rounded-xl bg-gray-900 text-white text-sm font-semibold hover:bg-black transition-all">+ Thêm học vấn</button>
                    </div>

                    <div id="educationRows_<?php echo e($job->id); ?>" class="space-y-4"></div>
                ...
                <div class="rounded-2xl border border-gray-100 bg-gray-50/40 p-5">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <button type="button" onclick="addWorkRow_<?php echo e($job->id); ?>()" class="px-4 py-2 rounded-xl bg-gray-900 text-white text-sm font-semibold hover:bg-black transition-all">+ Thêm vai trò</button>
                    </div>

                    <div id="workRows_<?php echo e($job->id); ?>" class="space-y-4"></div>
                    ...
                    <div class="mt-3 p-4 bg-white rounded-2xl border border-gray-100">
                        ...
                        <div id="workVerbChips_<?php echo e($job->id); ?>" class="mt-3 flex flex-wrap gap-2"></div>
                    </div>
                </div>
                ...
                <div class="rounded-2xl border border-gray-100 bg-gray-50/40 p-5">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="flex gap-2">
                            <button type="button" onclick="addSkillRow_<?php echo e($job->id); ?>('hard')" class="px-4 py-2 rounded-xl bg-indigo-50 text-indigo-700 text-sm font-semibold hover:bg-indigo-600 hover:text-white transition-all">+ Hard</button>
                            <button type="button" onclick="addSkillRow_<?php echo e($job->id); ?>('soft')" class="px-4 py-2 rounded-xl bg-purple-50 text-purple-700 text-sm font-semibold hover:bg-purple-600 hover:text-white transition-all">+ Soft</button>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <div class="rounded-2xl bg-white border border-gray-200 p-4">
                            ...
                            <div id="hardSkillRows_<?php echo e($job->id); ?>" class="space-y-3"></div>
                            <div class="mt-3">
                                <p class="text-xs font-semibold text-gray-700 mb-2">💡 Gợi ý theo công việc:</p>
                                <div id="hardSkillChips_<?php echo e($job->id); ?>" class="flex flex-wrap gap-2"></div>
                            </div>
                        </div>
                        ...
                        <div class="rounded-2xl bg-white border border-gray-200 p-4">
                            ...
                            <div id="softSkillRows_<?php echo e($job->id); ?>" class="space-y-3"></div>
                            <div class="mt-3">
                                <p class="text-xs font-semibold text-gray-700 mb-2">💡 Gợi ý theo công việc:</p>
                                <div id="softSkillChips_<?php echo e($job->id); ?>" class="flex flex-wrap gap-2"></div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex items-center justify-end gap-3">
                    <button type="button" onclick="closeCvDialog_<?php echo e($job->id); ?>()" class="px-5 py-3 rounded-xl bg-gray-100 text-gray-700 font-semibold hover:bg-gray-200 transition-all">Đóng</button>
                    <button type="button" onclick="saveCvDialog_<?php echo e($job->id); ?>()" class="px-5 py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold hover:shadow-lg transition-all">Lưu CV</button>
                </div>
            </div>
        </div>
    </dialog>
    ...
    <script>
        const cvDialog = document.getElementById('cvDialog_<?php echo e($job->id); ?>');
        const applyForm = document.getElementById('applyForm_<?php echo e($job->id); ?>');
        const cvModeInput = document.getElementById('cv_mode_<?php echo e($job->id); ?>');
        const cvUploadSection = document.getElementById('cv_upload_section_<?php echo e($job->id); ?>');
        const cvFormSection = document.getElementById('cv_form_section_<?php echo e($job->id); ?>');
        const cvFileInput = document.getElementById('cv_file_<?php echo e($job->id); ?>');

        let educationIndex_<?php echo e($job->id); ?> = 0;
        let workIndex_<?php echo e($job->id); ?> = 0;
        let hardSkillIndex_<?php echo e($job->id); ?> = 0;
        let softSkillIndex_<?php echo e($job->id); ?> = 0;

        // Handle form submission with loading state
        if (applyForm) {
            applyForm.addEventListener('submit', function(e) {
                const submitBtn = document.getElementById('submitBtn_<?php echo e($job->id); ?>');
                const submitText = document.getElementById('submitText_<?php echo e($job->id); ?>');
                const submitIcon = document.getElementById('submitIcon_<?php echo e($job->id); ?>');
                const submitSpinner = document.getElementById('submitSpinner_<?php echo e($job->id); ?>');
                const scoringProgress = document.getElementById('scoringProgress_<?php echo e($job->id); ?>');
        ...
        const strongVerbs_<?php echo e($job->id); ?> = [
            'Chủ trì',
            'Vận hành',
            'Tối ưu hóa',
            'Thiết kế',
        ...
        let lastWorkDescriptionEl_<?php echo e($job->id); ?> = null;

        let hydratedCvDialog_<?php echo e($job->id); ?> = false;

        const initialCvMode_<?php echo e($job->id); ?> = <?php echo json_encode(old('cv_mode', 'upload'), 512) ?>;
        const hasFormErrors_<?php echo e($job->id); ?> = <?php echo json_encode($errors->has('self_description') || $errors->has('education_json') || $errors->has('education_proofs') || $errors->has('education_proofs.*') || $errors->has('work_experiences_json') || $errors->has('skills_json'), 15, 512) ?>;
        ...
        function renderVerbChips(containerEl, getTargetTextarea) {
            ...
            strongVerbs_<?php echo e($job->id); ?>.forEach((verb) => {
        ...
        function getSkillSuggestions_<?php echo e($job->id); ?>(sourceText) {
            const title = normalizeTitle(sourceText ?? <?php echo json_encode($job->title, 15, 512) ?>);
            // Default suggestions
        ...
        function openCvDialog_<?php echo e($job->id); ?>() {
            ...
            if (!hydratedCvDialog_<?php echo e($job->id); ?>) {
                hydrateCvDialogFromHidden_<?php echo e($job->id); ?>();
                hydratedCvDialog_<?php echo e($job->id); ?> = true;
            }

            if (document.getElementById('educationRows_<?php echo e($job->id); ?>')?.children?.length === 0) addEducationRow_<?php echo e($job->id); ?>();
            if (document.getElementById('workRows_<?php echo e($job->id); ?>')?.children?.length === 0) addWorkRow_<?php echo e($job->id); ?>();
            if (document.getElementById('hardSkillRows_<?php echo e($job->id); ?>')?.children?.length === 0) addSkillRow_<?php echo e($job->id); ?>('hard');
            if (document.getElementById('softSkillRows_<?php echo e($job->id); ?>')?.children?.length === 0) addSkillRow_<?php echo e($job->id); ?>('soft');
        }

        function safeJsonParse_<?php echo e($job->id); ?>(text, fallback) {
        ...
        function hydrateCvDialogFromHidden_<?php echo e($job->id); ?>() {
            // Self description
            const hiddenSelf = document.getElementById('self_description_<?php echo e($job->id); ?>')?.value ?? '';
            const selfEl = document.getElementById('selfDescriptionInput_<?php echo e($job->id); ?>');
            if (selfEl && (selfEl.value ?? '').trim() === '' && (hiddenSelf ?? '').trim() !== '') {
                selfEl.value = hiddenSelf;
            }

            // Certifications & English
            const certHidden = document.getElementById('certifications_json_<?php echo e($job->id); ?>')?.value ?? '';
            const certData = safeJsonParse_<?php echo e($job->id); ?>(certHidden, {});
            
            const englishEl = document.getElementById('englishLevel_<?php echo e($job->id); ?>');
            if (englishEl && certData?.english_level) englishEl.value = certData.english_level;
            
            const toeicEl = document.getElementById('toeicScore_<?php echo e($job->id); ?>');
            if (toeicEl && certData?.toeic_score) toeicEl.value = certData.toeic_score;
            
            const ieltsEl = document.getElementById('ieltsScore_<?php echo e($job->id); ?>');
            if (ieltsEl && certData?.ielts_score) ieltsEl.value = certData.ielts_score;
            
            const yearsEl = document.getElementById('yearsOfExperience_<?php echo e($job->id); ?>');
            if (yearsEl && certData?.years_experience) yearsEl.value = certData.years_experience;
            
            const certsSelect = document.getElementById('certifications_<?php echo e($job->id); ?>');
            ...
            // Education
            const eduHidden = document.getElementById('education_json_<?php echo e($job->id); ?>')?.value ?? '';
            const edu = safeJsonParse_<?php echo e($job->id); ?>(eduHidden, []);
            const eduContainer = document.getElementById('educationRows_<?php echo e($job->id); ?>');
            if (eduContainer) {
                eduContainer.innerHTML = '';
                educationIndex_<?php echo e($job->id); ?> = 0;
                if (Array.isArray(edu) && edu.length > 0) {
                    edu.forEach((item) => {
                        addEducationRow_<?php echo e($job->id); ?>();
            ...
            // Work experiences
            const workHidden = document.getElementById('work_experiences_json_<?php echo e($job->id); ?>')?.value ?? '';
            const work = safeJsonParse_<?php echo e($job->id); ?>(workHidden, []);
            const workContainer = document.getElementById('workRows_<?php echo e($job->id); ?>');
            if (workContainer) {
                workContainer.innerHTML = '';
                workIndex_<?php echo e($job->id); ?> = 0;
                if (Array.isArray(work) && work.length > 0) {
                    work.forEach((item) => {
                        addWorkRow_<?php echo e($job->id); ?>();
            ...
            // Skills
            const skillsHidden = document.getElementById('skills_json_<?php echo e($job->id); ?>')?.value ?? '';
            const skills = safeJsonParse_<?php echo e($job->id); ?>(skillsHidden, { hard: [], soft: [] });
            const hardBox = document.getElementById('hardSkillRows_<?php echo e($job->id); ?>');
            const softBox = document.getElementById('softSkillRows_<?php echo e($job->id); ?>');

            if (hardBox) {
                hardBox.innerHTML = '';
                hardSkillIndex_<?php echo e($job->id); ?> = 0;
                const items = Array.isArray(skills?.hard) ? skills.hard : [];
                items.forEach((s) => addSkillRow_<?php echo e($job->id); ?>('hard', s?.name ?? '', String(s?.level ?? '3')));
            }
            if (softBox) {
                softBox.innerHTML = '';
                softSkillIndex_<?php echo e($job->id); ?> = 0;
                const items = Array.isArray(skills?.soft) ? skills.soft : [];
                items.forEach((s) => addSkillRow_<?php echo e($job->id); ?>('soft', s?.name ?? '', String(s?.level ?? '3')));
            }
        }

        function closeCvDialog_<?php echo e($job->id); ?>() {
            if (cvDialog && typeof cvDialog.close === 'function') cvDialog.close();
        }

        function setCvMode_<?php echo e($job->id); ?>(mode) {
        ...
        function addEducationRow_<?php echo e($job->id); ?>() {
            const container = document.getElementById('educationRows_<?php echo e($job->id); ?>');
            if (!container) return;
            const idx = educationIndex_<?php echo e($job->id); ?>++;
                ...
                <div class="flex items-start justify-between gap-3">
                    <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="file" name="education_proofs[]" form="applyForm_<?php echo e($job->id); ?>" accept=".pdf,.jpg,.jpeg,.png" class="block w-full text-sm text-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-600 hover:file:text-white">
                        </div>
                    </div>
                    <button type="button" class="px-3 py-2 rounded-xl bg-gray-100 text-gray-600 hover:bg-red-600 hover:text-white transition-all" onclick="removeEducationRow_<?php echo e($job->id); ?>(this)">Xóa</button>
                </div>
            `;
        ...
        function removeEducationRow_<?php echo e($job->id); ?>(btn) {
            const row = btn?.closest('[data-idx]');
            if (row) row.remove();
        }

        function addWorkRow_<?php echo e($job->id); ?>() {
            const container = document.getElementById('workRows_<?php echo e($job->id); ?>');
            if (!container) return;
            const idx = workIndex_<?php echo e($job->id); ?>++;
            ...
            row.innerHTML = `
                <div class="flex items-start justify-between gap-3">
                    ...
                    <button type="button" class="px-3 py-2 rounded-xl bg-gray-100 text-gray-600 hover:bg-red-600 hover:text-white transition-all" onclick="removeWorkRow_<?php echo e($job->id); ?>(this)">Xóa</button>
                </div>
            `;
            ...
            descEl?.addEventListener('focus', () => {
                lastWorkDescriptionEl_<?php echo e($job->id); ?> = descEl;
            });
            ...
            const updateSuggestionsFromPosition = () => {
                ...
                renderSkillChips_<?php echo e($job->id); ?>(text);
            };
        ...
        function removeWorkRow_<?php echo e($job->id); ?>(btn) {
        ...
        function addSkillRow_<?php echo e($job->id); ?>(type, presetName = '', presetLevel = '3') {
            const container = document.getElementById((type === 'hard' ? 'hardSkillRows_' : 'softSkillRows_') + '<?php echo e($job->id); ?>');
            if (!container) return;
            const idx = (type === 'hard') ? (hardSkillIndex_<?php echo e($job->id); ?>++) : (softSkillIndex_<?php echo e($job->id); ?>++);
            ...
            row.innerHTML = `
                ...
                <button type="button" class="px-3 py-2 rounded-xl bg-gray-100 text-gray-600 hover:bg-red-600 hover:text-white transition-all" onclick="removeSkillRow_<?php echo e($job->id); ?>(this)">Xóa</button>
            `;
            container.appendChild(row);
        }

        function removeSkillRow_<?php echo e($job->id); ?>(btn) {
            const row = btn?.closest('[data-sidx]');
            if (row) row.remove();
        }

        function renderSkillChips_<?php echo e($job->id); ?>(sourceText) {
            const suggestions = getSkillSuggestions_<?php echo e($job->id); ?>(sourceText);
            const hardBox = document.getElementById('hardSkillChips_<?php echo e($job->id); ?>');
            const softBox = document.getElementById('softSkillChips_<?php echo e($job->id); ?>');
            const render = (box, items, type) => {
                ...
                items.forEach((name) => {
                    ...
                    btn.addEventListener('click', () => addSkillRow_<?php echo e($job->id); ?>(type, name, '3'));
                    box.appendChild(btn);
                });
            };
        ...
        function saveCvDialog_<?php echo e($job->id); ?>(closeAfterSave = true) {
            console.log('[saveCvDialog] Function called, closeAfterSave:', closeAfterSave);
            
            const selfDesc = document.getElementById('selfDescriptionInput_<?php echo e($job->id); ?>')?.value ?? '';
            
            // Certifications - collect from dialog or fallback to hidden input
            const englishLevel = document.getElementById('englishLevel_<?php echo e($job->id); ?>')?.value ?? '';
            const toeicScore = document.getElementById('toeicScore_<?php echo e($job->id); ?>')?.value ?? '';
            const ieltsScore = document.getElementById('ieltsScore_<?php echo e($job->id); ?>')?.value ?? '';
            const yearsExp = document.getElementById('yearsOfExperience_<?php echo e($job->id); ?>')?.value ?? '';
            const certsSelect = document.getElementById('certifications_<?php echo e($job->id); ?>');
            ...
            if (!hasCertData) {
                const existingCerts = document.getElementById('certifications_json_<?php echo e($job->id); ?>')?.value;
            ...
            const rows = Array.from(document.querySelectorAll('#educationRows_<?php echo e($job->id); ?> [data-idx]'));
            ...
            if (rows.length > 0) {
                ...
                const existingEdu = document.getElementById('education_json_<?php echo e($job->id); ?>')?.value;
            ...
            const workRows = Array.from(document.querySelectorAll('#workRows_<?php echo e($job->id); ?> [data-widx]'));
            ...
            if (workRows.length > 0) {
                ...
                // Dialog is closed, use existing hidden input data
                const existingWork = document.getElementById('work_experiences_json_<?php echo e($job->id); ?>')?.value;
            ...
            const hardSkillRows = Array.from(document.querySelectorAll('#hardSkillRows_<?php echo e($job->id); ?> [data-sidx]'));
            const softSkillRows = Array.from(document.querySelectorAll('#softSkillRows_<?php echo e($job->id); ?> [data-sidx]'));
            ...
            if (skills.hard.length === 0 && skills.soft.length === 0) {
                const existingSkills = document.getElementById('skills_json_<?php echo e($job->id); ?>')?.value;
            ...
            document.getElementById('self_description_<?php echo e($job->id); ?>').value = selfDesc;
            document.getElementById('education_json_<?php echo e($job->id); ?>').value = JSON.stringify(education);
            document.getElementById('work_experiences_json_<?php echo e($job->id); ?>').value = JSON.stringify(work);
            document.getElementById('skills_json_<?php echo e($job->id); ?>').value = JSON.stringify(skills);
            document.getElementById('certifications_json_<?php echo e($job->id); ?>').value = JSON.stringify(certifications);

            if (closeAfterSave) {
                closeCvDialog_<?php echo e($job->id); ?>();
            }
        }

        function highlightCvDialogErrors_<?php echo e($job->id); ?>() {
            const selfDescEl = document.getElementById('selfDescriptionInput_<?php echo e($job->id); ?>');
            if (<?php echo json_encode($errors->has('self_description'), 15, 512) ?>) {
                selfDescEl?.classList.add('border-red-400');
            }
        }

        // Init chips
        renderVerbChips(document.getElementById('workVerbChips_<?php echo e($job->id); ?>'), () => {
            return lastWorkDescriptionEl_<?php echo e($job->id); ?> ?? document.querySelector('#workRows_<?php echo e($job->id); ?> [data-widx]:last-child [data-wfield="description"]');
        });
        renderSkillChips_<?php echo e($job->id); ?>(<?php echo json_encode($job->title, 15, 512) ?>);

        // Set initial mode based on old input
        setCvMode_<?php echo e($job->id); ?>(initialCvMode_<?php echo e($job->id); ?>);

        // Auto-open CV dialog if user chose CV nhanh and there are validation errors
        if (initialCvMode_<?php echo e($job->id); ?> === 'form' && hasFormErrors_<?php echo e($job->id); ?>) {
            openCvDialog_<?php echo e($job->id); ?>();
            highlightCvDialogErrors_<?php echo e($job->id); ?>();
        }
        ...
        if (applyForm) {
            applyForm.addEventListener('submit', function (e) {
                if (cvModeInput && cvModeInput.value === 'form') {
                    ...
                    saveCvDialog_<?php echo e($job->id); ?>(false);
                    
                    // Debug: Log what was saved
                    setTimeout(() => {
                        const eduData = document.getElementById('education_json_<?php echo e($job->id); ?>')?.value;
                        const workData = document.getElementById('work_experiences_json_<?php echo e($job->id); ?>')?.value;
                        const skillsData = document.getElementById('skills_json_<?php echo e($job->id); ?>')?.value;
        ...
        if (cvFileInput) {
            ...
            cvFileInput.addEventListener('change', function(e) {
                ...
                const placeholder = document.getElementById('upload-placeholder_<?php echo e($job->id); ?>');
                const success = document.getElementById('upload-success_<?php echo e($job->id); ?>');
                const fileNameDisplay = document.getElementById('file-name-display_<?php echo e($job->id); ?>');
        ...
        function submitApplicationForm_<?php echo e($job->id); ?>() {
            ...
            // Submit the form
            const form = document.getElementById('applyForm_<?php echo e($job->id); ?>');
            if (form) {
...
<?php endif; ?>
<?php /**PATH D:\web\cpanel_public_html\backend\resources\views/jobs/show.blade.php ENDPATH**/ ?>